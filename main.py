...
# =========================
# ROLLIN JATT EMOTE BOT – MAIN (with Website Bridge)
# =========================
...
# --- Your existing imports MUST remain available in your project ---
# (proto files, helpers, packet builders, etc.)
# Example:
# from your_packets import (
#     EnC_PacKeT, DecodE_HeX, DeCode_PackEt, GenJoinSquadsPacket, Emote_k,
#     xSEndMsg, xSEndMsgsQ, AuthClan, OpEnSq, cHSq, SEnd_InV, ExiT, FS,
#     GeTSQDaTa, AutH_Chat, get_random_color, xMsGFixinG, equie_emote,
#     GeNeRaTeAccEss, EncRypTMajoRLoGin, MajoRLoGinrEs_pb2, PorTs_pb2,
#     DEcwHisPErMsG_pb2
# )
# And any global headers like:
# Hr = {...}
...
# Writers shared by TCP tasks
online_writer = None
whisper_writer = None

# ---------------------------------------
# >>> YOUR EXISTING API HELPERS (kept) <<<
# ---------------------------------------
            ...
            while True:
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        JoinCHaT = await AutH_Chat(3, OwNer_UiD, CHaT_CoDe, GLOBALS["key"], GLOBALS["iv"])
                        await SEndPacKeT(whisper_writer, online_writer, 'ChaT', JoinCHaT)

                        message = f'[B][C]{get_random_color()}\n- WeLComE To Emote Bot ! '
                        P = await SEndMsG(0, message, OwNer_UiD, OwNer_UiD, GLOBALS["key"], GLOBALS["iv"])
...
# WEBSITE API (aiohttp)
# =========================
async def http_send_emote(request: web.Request):
    """
    POST /send_emote
    JSON:
    {
      "team_code": "237474" OR "/x/237474",
      "emote_id": 909000098,
      "uids": ["505118735","..."]   // 1 to 4 UIDs
    }
    """
    try:
        body = await request.json()
        team_code = (body.get("team_code") or "").strip()
        emote_id = int(body.get("emote_id"))
        uids_in = body.get("uids") or []

        if team_code.startswith("/x/"):
            team_code = team_code.split("/x/")[1].strip()

        key = GLOBALS["key"]
        iv = GLOBALS["iv"]
        region = GLOBALS["region"]
        if not (key and iv and region and online_writer):
            return web.json_response({"ok": False, "error": "Bot not ready/online"}, status=503)

        if not team_code:
            return web.json_response({"ok": False, "error": "Missing team_code"}, status=400)
        if not uids_in:
            return web.json_response({"ok": False, "error": "No UIDs provided"}, status=400)

        print(f"🌐 /send_emote -> team:{team_code} emote:{emote_id} uids:{uids_in}")
        ...
        for u in uids_in:
            try:
                uid_val = int(u)
                H = await Emote_k(uid_val, emote_id, key, iv, region)
                await SEndPacKeT(whisper_writer, online_writer, 'OnLine', H)
                sent_to.append(uid_val)
            except Exception as e:
                print(f"Emote error for UID {u}: {e}")

        return web.json_response({"ok": True, "team_code": team_code, "emote_id": emote_id, "sent_to": sent_to},
                                 headers={"Access-Control-Allow-Origin": "*"})

    except Exception as e:
        print("HTTP /send_emote error:", e)
        return web.json_response({"ok": False, "error": str(e)}, status=500)

async def http_health(request: web.Request):
    ready = bool(GLOBALS["key"] and GLOBALS["iv"] and GLOBALS["region"] and online_writer)
...
async def http_options_handler(request: web.Request):
...
async def start_http_server():
    ...
    app.router.add_static('/static', path=os.path.abspath('./static'), show_index=False)

    # API routes
    app.router.add_route("OPTIONS", "/send_emote", http_options_handler)
    app.router.add_post("/send_emote", http_send_emote)
    app.router.add_get("/health", http_health)

    runner = web.AppRunner(app)
    await runner.setup()
    site = web.TCPSite(runner, HTTP_HOST, HTTP_PORT)
    print(f"🌐 Website running at http://{HTTP_HOST}:{HTTP_PORT}")
    await site.start()
...
# =========================
async def MaiiiinE():
    # You told me to keep these (hardcoded)
    Uid, Pw = '4261873563','EF54298AD39DC24E807782671993C14A8E544B9F1AE279E15CEC9C8227CBD96A'
    ...
    task_chat = asyncio.create_task(TcPChaT(ChaTiP, ChaTporT, AutHToKen, key, iv, LoGinDaTaUncRypTinG, ready_event, region))
    await ready_event.wait()
    await asyncio.sleep(1)

    task_online = asyncio.create_task(TcPOnLine(OnLineiP, OnLineporT, key, iv, AutHToKen))
    task_http = asyncio.create_task(start_http_server())

    os.system('clear')
    try:
        from pyfiglet import figlet_format as render
        print(render('WINTER'))
    except:
        print('WINTER')
    ...
    await asyncio.gather(task_chat, task_online, task_http)
...
if __name__ == "__main__":
    import asyncio
    async def main():
        # Run both the HTTP server and your bot together
        await asyncio.gather(
            start_http_server(),  # 🌐 starts aiohttp web server
            StarTinG()            # 🤖 starts your bot loop
        )
